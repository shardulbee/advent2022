...
let value choice = match choice with Rock -> 1 | Paper -> 2 | Scissors -> 3

let result_of (game : game) =
  match (game.opponent, game.mine) with
  | Rock, Rock -> Draw
  | Paper, Paper -> Draw
...
let game_for_result (game : game_result) =
  let my_choice =
    match (game.opponent, game.result) with
    | Rock, Win -> Paper
    | Rock, Loss -> Scissors
    | Paper, Win -> Scissors
    | Paper, Loss -> Rock
...
let rec parse_lines parse_func lines =
  match lines with
  | [] -> []
